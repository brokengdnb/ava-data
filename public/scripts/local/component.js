angular.module('localCtrl', [
 'ngDroplet'
]);

angularApp.controller('localCtrl', function searchCtrl($rootScope, $scope, $mdToast, $mdDialog, $http, Notes, $location, $timeout) {
    $http.get('/listLocal').then(function(data) {
    ...
    $scope.localImage = "/uploads/local/tn.png";
    ...
    $scope.getVideoInfo = function (path) {
        $http.get("/getLocalInfo?path=" + path.name).then(function(response) {
            $scope.dataPost.youtubeOriginal = path.name;

            $scope.dataPost.content = $scope.dataPost.youtubeOriginal;
...
// creates : values = ["Artist","Title"]
    ...
    $scope.convertVideo = function() {
        ...
        $scope.dataPost.youtubeID = $scope.dataPost.youtubeOriginal;


        $http.post("/convertVideo", {'data': $scope.dataPost}).then(function (data, status, headers, config) {
            console.log(data);
    ...
    $scope.convertAudio = function() {
        ...
        $http.post("/convertAudio", {'data': $scope.dataPost}).then(function (data, status, headers, config) {
            console.log(data);
    ...
    $scope.convertPSP = function() {
        $scope.determinateValue = 0;
        $scope.determinateValue2 = 0;

        $scope.disableInputs = true;
        $scope.convertType = "psp";

        $scope.dataPost.youtubeID = $scope.dataPost.youtubeOriginal;

        $http.post("/convertPSP", {'data': $scope.dataPost}).then(function (data, status, headers, config) {
            console.log(data);
